...
export async function saveSessionToFirestore(sessionData) {
  try {
    ...
    await addDoc(collection(db, "sessions"), {
      ...
      fullUrl: url,
      recordedAt: Timestamp.fromMillis(Number(timestamp))
    });
  ...
  try {
    ...
    const sessions = querySnapshot.docs.map(doc => {
      ...
      return {
        ...
        url: data.fullUrl || '',
        timestamp: data.recordedAt && typeof data.recordedAt.toMillis === 'function' ? data.recordedAt.toMillis() : data.recordedAt || ''
      };
    });
    return sessions;
  } catch (error) {
